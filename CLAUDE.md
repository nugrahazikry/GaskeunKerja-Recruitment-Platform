...
**⚠️ Scope, tested 2026-07-13 — read before reaching for this tool.** Graphify is genuinely useful for **code-structure navigation** (where a function lives, what a module imports, what calls what) — verified working: `graphify query "how does the CV parsing pipeline connect to candidate creation"` correctly surfaced `ingest_cv()` and its real call chain. **It does NOT resolve foreign-key / data-model relationships.** Tested directly: `graphify path "Job" "Candidate"`, `"Job" "JDCompetency"`, and `"Candidate" "MatchScore"` all returned the same unhelpful answer — both classes routed through `Base` (the shared SQLAlchemy declarative class) via an `INFERRED` edge, never the real FK column relationship (e.g. `candidates.job_id → jobs.id`). A broader query for "what's the FK deletion order across these 8 tables" returned 126 mostly-irrelevant nodes with no usable answer. This is because graphify's extraction is AST/import-level — it doesn't parse `ForeignKey(...)` column definitions as a distinct relationship type. **For FK/deletion-order/schema-relationship questions, read the model files directly** (`grep ForeignKey backend/models/*.py` or open them) — this is faster and correct on the first try, unlike graphify for this specific question type.

Rules:
- **Reach for graphify**: locating a function/module, tracing call chains, "where is X defined," understanding imports — code-structure questions.
- **Don't reach for graphify**: FK/deletion-order questions, database schema relationships, anything about data-model semantics. Read the model/schema files directly instead.
- Invoke it when the question actually matches what it's good at, not reflexively "because it's set up" — that reflexive habit was tried and found wanting for the FK case above.
